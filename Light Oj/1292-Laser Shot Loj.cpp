...
#define inpd(x,y) scanf("%lf%lf",&x,&y)
...
/// ************************************** Code starts here ****************************************** */
string s,s1;
ll cs=0,counT=0,k,ans=0,l=0,sum1=0,sum=0,Min,num;
    ...
    f1(k,t)
    {
       ...
       int Max = -10000;
        ...
        f0(i,n)
        {
            ...
            for(int j=i+1; j<n; j++)
            {
                double val = Slope(vc[j],vc[i]);
                slope[val]++;
                Max = max(Max, slope[val]);
            }
        }
        if(Max<0)Max = 0;
        pf("Case %d: %d\n",k,Max+1);
